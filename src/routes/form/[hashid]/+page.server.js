		...
		try {
			...
			if (token) {
				try {
					...
					// Use a Set to track sent tags and prevent duplicates
							...
							if (submittedValue) {
								...
								for (const val of valuesToProcess) {
									...
									if (option && option.tag) {
										// Only send if we haven't sent this tag yet (deduplication)
										if (!sentTags.has(option.tag)) {
											await sendTagToAPI(option.tag, field.label);
											sentTags.add(option.tag);
										}
									}
								}
							}
